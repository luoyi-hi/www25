...
def get_common_params():
    ...
    parser.add_argument('--train_path', default='XXXXXXXXX/SynRTP/data/dataset/cq_dataset/train.npy',type=str, help='the path of the original train dataset')
    parser.add_argument('--val_path', default='XXXXXXXXX/SynRTP/data/dataset/cq_dataset/val.npy',type=str, help='the path of the original val dataset')
    parser.add_argument('--test_path', default='XXXXXXXXX/SynRTP/data/dataset/cq_dataset/test.npy', type=str, help='the path of the original test dataset')
...
def calculate_graphormer_features(A_matrix, E_features, N_nodes, max_shortest_path_len):
    ...
    shortest_path_distances = np.full((N_nodes, N_nodes), max_shortest_path_len + 1, dtype=np.int32)
    shortest_path_edge_features_aggregated = np.zeros((N_nodes, N_nodes, E_features.shape[2]), dtype=np.float32)
    ...
    for i in range(N_nodes):
        ...
        while head < len(queue): 
            ...
            for neighbor in G.neighbors(curr_node):
                if neighbor not in visited:
                    ...
                    if neighbor not in target_shortest_paths or new_dist < target_shortest_paths[neighbor][0]:
                        ...
                        if len(new_path_nodes) > 1:
                            for k_path in range(len(new_path_nodes) - 1):
                                ...
                                if 'feature' in G[src_node][dest_node]:
                                    current_path_edge_features_list.append(G[src_node][dest_node]['feature'])
                                else:
                                    print(f"Warning: Edge ({src_node}, {dest_node}) missing 'feature' in NetworkX graph.")
                                    current_path_edge_features_list.append(np.zeros(E_features.shape[2], dtype=np.float32))

                        target_shortest_paths[neighbor] = (new_dist, current_path_edge_features_list)
        ...
        for target_node, (dist, edge_feat_list) in target_shortest_paths.items():
            if dist <= max_shortest_path_len: 
                ...
                if edge_feat_list: 
                    shortest_path_edge_features_aggregated[i, target_node, :] = np.mean(edge_feat_list, axis=0)
                else:
                    shortest_path_edge_features_aggregated[i, target_node, :] = np.zeros(E_features.shape[2], dtype=np.float32)
        
    return node_in_degree, node_out_degree, shortest_path_distances, shortest_path_edge_features_aggregated

def prepare_graphormer_input_features(A_batch_np, E_features_batch_np, N_nodes, max_shortest_path_len):
    ...
    all_shortest_path_distances_flat = np.empty((B_flat, N_nodes, N_nodes), dtype=np.int32)
    all_shortest_path_edge_features_aggregated_flat = np.empty((B_flat, N_nodes, N_nodes, d_e), dtype=np.float32)

    for idx in range(B_flat):
        in_d, out_d, spd, sp_edge_agg = calculate_graphormer_features(
            A_flat_np[idx],
            E_features_flat_np[idx],
            N_nodes,
    ...
    shortest_path_distances = all_shortest_path_distances_flat.reshape(B, T, N_nodes, N_nodes)
    shortest_path_edge_features_aggregated = all_shortest_path_edge_features_aggregated_flat.reshape(B, T, N_nodes, N_nodes, d_e)

    return (node_in_degree, node_out_degree, 
            shortest_path_distances, shortest_path_edge_features_aggregated)


def process_and_save_dataset(dataloader, dataset_name, N_nodes, max_shortest_path_len, output_dir):
    ...
    for i, batch in enumerate(tqdm(dataloader, total=total_batches, desc=f"Processing {dataset_name}")):
        V, V_reach_mask, E, start_fea, start_idx, cou_fea, route_label, label_len, A, V_len, time_label = batch2input(batch, 'cpu')
        ...
        batch_node_in_degree, batch_node_out_degree, \
        batch_shortest_path_distances, batch_shortest_path_edge_features_aggregated = \
            prepare_graphormer_input_features(A_np, E_np, N_nodes, max_shortest_path_len)
        end_time_batch_prep = time.time()
        
        if (i + 1) % 10 == 0 or (i + 1) == total_batches:
             print(f"  Batch {i+1}/{total_batches} Graphormer feature prep time: {end_time_batch_prep - start_time_batch_prep:.4f} seconds")
        ...
        collected_shortest_path_edge_features_aggregated.append(batch_shortest_path_edge_features_aggregated)

    print(f"\n--- Merging all batches for {dataset_name} dataset ---")
    final_dataset = {}
    
    if collected_V: 
        final_dataset['V'] = np.concatenate(collected_V, axis=0) # (Total_B_original, T, N, d_v)
        final_dataset['V_len'] = np.concatenate(collected_V_len, axis=0) # (Total_B_original, T)
        final_dataset['V_reach_mask'] = np.concatenate(collected_V_reach_mask, axis=0) # (Total_B_original, T, N)
        final_dataset['E_static_fea'] = np.concatenate(collected_E_static_fea, axis=0) # (Total_B_original, T, N, N, d_e)
        final_dataset['A'] = np.concatenate(collected_A, axis=0) # (Total_B_original, T, N, N)
        final_dataset['start_fea'] = np.concatenate(collected_start_fea, axis=0) # (Total_B_original, T, d_s)
        final_dataset['start_idx'] = np.concatenate(collected_start_idx, axis=0) # (Total_B_original, T)
        final_dataset['cou_fea'] = np.concatenate(collected_cou_fea, axis=0) # (Total_B_original, T, d_w_orig)
        final_dataset['route_label'] = np.concatenate(collected_route_label, axis=0) # (Total_B_original, T, N)
        final_dataset['label_len'] = np.concatenate(collected_label_len, axis=0) # (Total_B_original, T)
        final_dataset['time_label'] = np.concatenate(collected_time_label, axis=0) # (Total_B_original, T)

        final_dataset['node_in_degree'] = np.concatenate(collected_node_in_degree, axis=0) # (Total_B_original, T, N)
        final_dataset['node_out_degree'] = np.concatenate(collected_node_out_degree, axis=0) # (Total_B_original, T, N)
        final_dataset['shortest_path_distances'] = np.concatenate(collected_shortest_path_distances, axis=0) # (Total_B_original, T, N, N)
        final_dataset['shortest_path_edge_features_aggregated'] = np.concatenate(collected_shortest_path_edge_features_aggregated, axis=0) # (Total_B_original, T, N, N, d_e)
    else:
        print(f"Warning: No data collected for {dataset_name}. Skipping save.")
        return 

    output_filename = os.path.join(output_dir, f"{dataset_name}_SynRPT.npy")
...
if __name__ == "__main__":
    ...
    print(f'Number of Train Samples:{len(train_dataset)} | Number of val Samples:{len(val_dataset)} | Number of test Samples:{len(test_dataset)}')
    N_NODES = params['max_task_num'] 
    MAX_SPD_LEN = params['max_shortest_path_len'] 

    output_base_dir = os.path.join(ws, f'processed_data/{params["dataset"]}')
    ...
    process_and_save_dataset(train_loader, 'train', N_NODES, MAX_SPD_LEN, output_base_dir)
